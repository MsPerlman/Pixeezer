...
internal class QueueItemDismissGestureHandler(
    ...
    // Optional start-to-end (positive / right) swipe → play next. When null the right direction
    // is blocked and left-swipe-to-dismiss behaves exactly as before.
    ...
    fun onHorizontalDrag(dragAmount: Float) {
        ...
        // Right swipe is only allowed when a right action is wired; otherwise clamp to 0 as before.
        if (accumulatedDragX > 0f && onSwipeRight == null) {
            accumulatedDragX = 0f
        ...
        when (dragPhase) {
            QueueDismissDragPhase.TENSION -> {
                ...
                if (abs(accumulatedDragX) < tensionThresholdPx) {
                    ...
                    val tensionOffset = maxTensionOffsetPx * dragFraction * sign(accumulatedDragX)
                    scope.launch {
    ...
    fun onDragEnd() {
        ...
        val dismissThreshold = itemWidthPx * 0.40f
        val committed = abs(accumulatedDragX) > dismissThreshold
        val swipedRight = accumulatedDragX > 0f

        if (committed && swipedRight && onSwipeRight != null) {
            // Play-next: fire the action and spring back (the row stays in the queue).
            performAppCompatHapticFeedback(
                hapticView,
                appHapticsConfig,
...
internal fun rememberQueueItemDismissGestureHandler(
    ...
    return remember(scope, density, hapticView, appHapticsConfig, offsetAnimatable, itemWidthPx, onDismiss, onSwipeRight) {
        QueueItemDismissGestureHandler(
            scope = scope,
            density = density,
