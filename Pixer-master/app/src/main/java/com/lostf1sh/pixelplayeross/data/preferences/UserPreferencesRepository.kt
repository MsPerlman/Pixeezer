...
constructor(
    ...
    private object PreferencesKeys {
        ...
        val PINNED_HOME_ITEMS = stringPreferencesKey("pinned_home_items")
        val PINNED_HOME_SEEDED = booleanPreferencesKey("pinned_home_seeded")
        ...
        val DEEZER_GATEWAY_ARL = stringPreferencesKey("deezer_gateway_arl")
    ...
    val deezerGatewayArlFlow: Flow<String?> = dataStore.data.map { preferences ->
        preferences[PreferencesKeys.DEEZER_GATEWAY_ARL]
    }

    suspend fun saveDeezerGatewayCredentials(arl: String) {
        dataStore.edit { preferences ->
            preferences[PreferencesKeys.DEEZER_GATEWAY_ARL] = arl
        }
    }

    suspend fun clearDeezerGatewayCredentials() {
        dataStore.edit { preferences ->
            preferences.remove(PreferencesKeys.DEEZER_GATEWAY_ARL)
        }
    }
    ...
    val pinnedHomeItemsFlow: Flow<List<PinnedHomeItem>> =
            dataStore.data.map { preferences ->
                val jsonString = preferences[PreferencesKeys.PINNED_HOME_ITEMS]
                if (jsonString != null) {
                    runCatching { json.decodeFromString<List<PinnedHomeItem>>(jsonString) }
    ...
    val pinnedHomeSeededFlow: Flow<Boolean> =
            dataStore.data.map { preferences -> preferences[PreferencesKeys.PINNED_HOME_SEEDED] ?: false }

    suspend fun togglePinnedHomeItem(item: PinnedHomeItem) {
        dataStore.edit { preferences ->
            val current = preferences[PreferencesKeys.PINNED_HOME_ITEMS]?.let { raw ->
                runCatching { json.decodeFromString<List<PinnedHomeItem>>(raw) }.getOrDefault(emptyList())
            } ?: emptyList()
            val alreadyPinned = current.any { it.type == item.type && it.id == item.id }
            val updated = if (alreadyPinned) {
                current.filterNot { it.type == item.type && it.id == item.id }
            } else {
                current + item
            }
            preferences[PreferencesKeys.PINNED_HOME_ITEMS] = json.encodeToString(updated)
        }
    }

    suspend fun removePinnedHomeItemsOfType(type: PinnedItemType) {
        dataStore.edit { preferences ->
            val current = preferences[PreferencesKeys.PINNED_HOME_ITEMS]?.let { raw ->
                runCatching { json.decodeFromString<List<PinnedHomeItem>>(raw) }.getOrDefault(emptyList())
            } ?: emptyList()
            if (current.none { it.type == type }) return@edit
            preferences[PreferencesKeys.PINNED_HOME_ITEMS] = json.encodeToString(current.filterNot { it.type == type })
        }
    }

    suspend fun seedPinnedHomeItemsIfNeeded(defaults: List<PinnedHomeItem>) {
        dataStore.edit { preferences ->
            if (preferences[PreferencesKeys.PINNED_HOME_SEEDED] == true) return@edit
            preferences[PreferencesKeys.PINNED_HOME_ITEMS] = json.encodeToString(defaults)
            preferences[PreferencesKeys.PINNED_HOME_SEEDED] = true
        }
    }
