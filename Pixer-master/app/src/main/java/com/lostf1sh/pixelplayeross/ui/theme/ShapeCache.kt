...
/**
 * Corner shape for Home/Library/Genre grid tiles. Preserves each call site's own pre-Deezer
 * shape (smooth squircle or plain rounded corner, per [smoothWhenDefault]) so non-Deezer
 * appearance is unchanged; converges to a flatter rounded-square under the Deezer skin.
 */
@Composable
fun rememberAppTileShape(cornerRadius: Dp, smoothWhenDefault: Boolean = true): Shape {
    val skin = LocalAppSkin.current
    return when {
        skin == AppSkin.DEEZER -> RoundedCornerShape(8.dp)
        smoothWhenDefault -> AbsoluteSmoothCornerShape(cornerRadius, 60)
