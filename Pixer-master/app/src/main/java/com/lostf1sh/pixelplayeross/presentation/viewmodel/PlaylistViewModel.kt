...
import com.lostf1sh.pixelplayeross.data.network.deezer.toSong
...
class PlaylistViewModel @Inject constructor(
    ...
    fun loadPlaylistDetails(playlistId: String) {
        viewModelScope.launch {
            ...
            try {
                if (playlistId.startsWith("deezer_")) {
                    val rawId = if (playlistId.startsWith("deezer_fav_")) {
                        playlistId.removePrefix("deezer_fav_")
                    } else {
                        playlistId.removePrefix("deezer_")
                    }
                    ...
                    if (response?.data != null) {
                        val pseudoPlaylist = Playlist(
                            ...
                            coverImageUri = response.data.attributes?.image?.large
                                ?: response.data.attributes?.image?.medium
                                ?: response.data.attributes?.image?.small,
                        ...
                        val fallbackSongsList = response.data.included.mapNotNull { track ->
                        ...
                        val songsList = if (gatewaySongs.isNotEmpty()) {
                            gatewaySongs.map { it.toSong() }
                        } else {
                            fallbackSongsList
                        }
                        val finalPlaylist = if (gatewaySongs.isNotEmpty()) {
                            pseudoPlaylist.copy(songIds = songsList.map { it.id })
                        } else {
                            pseudoPlaylist
                        }
                        ...
                        _uiState.update {
                            it.copy(
                                currentPlaylistDetails = finalPlaylist,
