...
internal val NavBarDeezerContentHeight = 72.dp
...
internal fun resolveNavBarContentHeight(compactMode: Boolean, deezerSkin: Boolean = false): Dp = when {
    compactMode -> NavBarCompactContentHeight
    // Deezer's bar is roughly a third shorter than Pixer's, while still showing labels.
    deezerSkin -> NavBarDeezerContentHeight
    else -> NavBarContentHeight
}

internal fun resolveMainScreenBottomGradientHeight(compactMode: Boolean, deezerSkin: Boolean = false): Dp =
    resolveNavBarContentHeight(compactMode, deezerSkin) + MainScreenBottomGradientExtraHeight

internal fun resolveNavBarSurfaceHeight(
    navBarStyle: String,
    systemNavBarInset: Dp,
    compactMode: Boolean,
    deezerSkin: Boolean = false
): Dp {
    val contentHeight = resolveNavBarContentHeight(compactMode, deezerSkin)
...
internal fun resolveNavBarOccupiedHeight(
    ...
    compactMode: Boolean,
    deezerSkin: Boolean = false
): Dp = resolveNavBarContentHeight(compactMode, deezerSkin) + systemNavBarInset
...
private fun PlayerInternalNavigationItemsRow(
    ...
    Row(
        ...
        navItems.forEach { item ->
            ...
            // The Deezer scheme's secondaryContainer is near-black, so the default indicator would
            // be invisible on the bar; use a translucent accent backdrop instead.
            val indicatorColorFromTheme = if (LocalAppSkin.current == AppSkin.DEEZER) {
